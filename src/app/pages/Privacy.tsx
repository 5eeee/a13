...
import { OFFICE_ADDRESS } from "../lib/contactsMap";
...
export function Privacy() {
  ...
  const sections = [
    ...baseSections,
    {
      ...
      text: `По вопросам обработки персональных данных обращайтесь: телефон ${s.phone}, электронная почта ${s.email}, адрес: ${s.address || OFFICE_ADDRESS}.`,
    },
  ];
        ...
        <div className="space-y-8">
          {sections.map((sec, i) => (
            <FadeIn key={i} delay={i * 0.04}>
              <div>
                <h2 className="text-gray-900 font-semibold text-lg mb-2">{sec.title}</h2>
                <p className="text-gray-500 text-sm leading-relaxed">{sec.text}</p>
              </div>
            </FadeIn>
          ))}
        </div>
